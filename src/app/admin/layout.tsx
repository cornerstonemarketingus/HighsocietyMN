    ...
    <div className="min-h-screen bg-slate-50 md:flex">
      ...
      <aside className="w-64 border-r border-slate-200 bg-white shrink-0 hidden md:flex flex-col">
        ...
        <nav className="flex-1 p-4 space-y-1">
          {adminLinks.map(({ href, label, icon: Icon }) => (
            <Link
              key={href}
              href={href}
              className="flex items-center gap-3 px-3 py-2.5 rounded-lg text-slate-700 hover:text-green-700 hover:bg-slate-50 transition-colors text-sm"
            >
        ...
        <div className="p-4 border-t border-slate-200">
          <Link href="/" className="text-xs text-slate-500 hover:text-green-700">
            ← Back to Store
          </Link>
        </div>
      </aside>
