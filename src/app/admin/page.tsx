...
export default async function AdminDashboard() {
  ...
  const cards = [
    { label: "Total Revenue", value: formatPrice(stats.revenue), icon: DollarSign, color: "text-green-700" },
    { label: "Total Orders", value: stats.orderCount.toString(), icon: ShoppingBag, color: "text-blue-400" },
    { label: "Products", value: stats.productCount.toString(), icon: Package, color: "text-green-700" },
            ...
            {recentOrders.map((order) => (
              <div key={order.id} className="p-4 flex items-center justify-between">
                ...
                <div className="text-right">
                  <p className="text-green-700 font-medium">{formatPrice(order.total)}</p>
