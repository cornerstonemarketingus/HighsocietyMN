      ...
      <main className="mx-auto max-w-5xl px-4 py-12 sm:px-6 lg:px-8">
        ...
        <div className="grid gap-8 lg:grid-cols-[1.2fr_0.8fr]">
              ...
              <div className="mt-3 grid grid-cols-2 gap-3">
                {(["DELIVERY", "PICKUP"] as const).map((option) => (
                  <button key={option} onClick={() => setMethod(option)}
                    className={`rounded-xl border p-4 text-left ${method === option ? "border-green-600 bg-green-50 text-green-800" : "border-slate-200"}`}>
          ...
          <aside className="h-fit rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
            ...
            {discountCode && <p className="mt-3 rounded-lg bg-green-50 px-3 py-2 text-sm font-medium text-green-700">Wheel reward: {discountCode}</p>}
