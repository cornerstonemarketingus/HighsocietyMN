  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-950">
      ...
      <main className="mx-auto max-w-6xl space-y-8 px-4 py-12 sm:px-6 lg:px-8">
        <div>
          <p className="text-sm font-semibold uppercase tracking-[0.2em] text-green-600">Member account</p>
        ...
        <section className="rounded-2xl border border-green-100 bg-green-50 p-6">
          ...
          <code className="mt-4 inline-block rounded-xl bg-white px-4 py-3 text-lg font-bold text-green-700">{rewards?.referralCode ?? "Loading…"}</code>
        </section>
          ...
          <div className="mt-5 divide-y divide-slate-100">
            ...
            {orders.map((order) => (
              <div key={order.id} className="flex flex-wrap items-center justify-between gap-3 py-4">
                ...
                <span className="rounded-full bg-green-50 px-3 py-1 text-sm font-medium text-green-700">{order.status}</span>
