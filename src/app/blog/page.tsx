          ...
          <div className="grid gap-8 sm:grid-cols-2 lg:grid-cols-3">
            {posts.map((post) => (
              <Link
                key={post.id}
                href={`/blog/${post.slug}`}
                className="group flex flex-col rounded-2xl border border-slate-200 bg-slate-50 overflow-hidden hover:border-orange-500/40 transition-all"
              >
                {post.image && (
                ...
                {!post.image && (
                  <div className="w-full h-48 bg-gradient-to-br from-orange-900/30 to-black/60 flex items-center justify-center">
                    <BookOpen className="h-12 w-12 text-orange-800" />
                  </div>
                )}
                <div className="flex flex-col flex-1 p-6 space-y-3">
                  ...
                  <h2 className="text-slate-950 font-semibold text-lg leading-snug group-hover:text-orange-400 transition-colors">
                    {post.title}
                  </h2>
                  {post.excerpt && (
                    <p className="text-slate-600 text-sm flex-1 line-clamp-3">{post.excerpt}</p>
                  )}
                  <span className="flex items-center gap-1 text-orange-400 text-sm mt-auto">
