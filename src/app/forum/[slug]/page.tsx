  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <main className="mx-auto max-w-3xl px-4 py-16 sm:px-6">
        <Link
          href="/forum"
          className="inline-flex items-center gap-2 text-slate-600 hover:text-emerald-400 text-sm mb-6"
        >
        ...
        <div className="mb-8">
          <div className="flex items-center gap-2 mb-2">
            {thread.pinned && <Pin className="h-4 w-4 text-emerald-500" />}
            {thread.locked && <Lock className="h-4 w-4 text-slate-500" />}
            <span className="text-xs text-emerald-500 font-medium">{thread.category.name}</span>
          </div>
        ...
        {thread.locked ? (
          ...
          <form onSubmit={handleReply} className="space-y-3">
            ...
            <textarea
              ...
              maxLength={5000}
              required
              className="w-full rounded-xl border border-slate-300 bg-slate-50 px-4 py-3 text-sm text-slate-950 placeholder:text-slate-500 focus:outline-none focus:ring-2 focus:ring-emerald-500 resize-none"
            />
