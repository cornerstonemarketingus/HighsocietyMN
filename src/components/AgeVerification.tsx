...
export function AgeVerification() {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center overflow-y-auto bg-emerald-950/80 p-4 backdrop-blur-xl">
      <div className="my-auto w-full max-w-3xl overflow-y-auto rounded-[2rem] border border-emerald-100 bg-white p-6 text-center shadow-2xl sm:p-9">
        <BrandMark className="mx-auto h-14 w-14" />
        <p className="mt-3 text-xs font-bold uppercase tracking-[0.3em] text-emerald-600">High Society Minnesota</p>

        {step === "age" && (
          <div className="mx-auto mt-7 max-w-xl">
            <div className="text-6xl font-bold text-emerald-700">21+</div>
            <h1 className="mt-4 text-3xl font-semibold text-slate-950">Are you 21 years of age or older?</h1>
            <p className="mt-4 text-sm leading-6 text-slate-600">You must be 21 or older to enter. By continuing, you confirm your age and agree to our <a href="/terms" className="font-medium text-emerald-700 underline">Terms of Service</a>.</p>
        ...
        {step === "choice" && (
          <div className="mx-auto mt-7 max-w-xl rounded-3xl border border-emerald-100 bg-emerald-50 p-6 text-left sm:p-8">
            <p className="text-sm font-semibold uppercase tracking-[0.2em] text-emerald-600">Private list</p>
            ...
            <form onSubmit={joinList}>
              ...
              <input id="gate-email" type="email" required value={email} onChange={(event) => setEmail(event.target.value)}
                className="mt-2 h-12 w-full rounded-xl border border-slate-300 bg-white px-4 text-slate-950 outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-100"
            ...
            <button type="button" onClick={enterSite} className="mt-4 w-full py-2 text-sm font-semibold text-slate-600 hover:text-emerald-700">No thanks, continue to the site</button>
