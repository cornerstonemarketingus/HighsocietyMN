...
export function Footer({ hideNewsletter = false }: { hideNewsletter?: boolean }) {
  return (
    <footer className="relative mt-20 overflow-hidden border-t border-white/[.07] bg-[#050505] text-white">
      ...
      <div className="pointer-events-none absolute -right-20 bottom-0 h-72 w-72 rounded-full bg-emerald-500/10 blur-[110px]" />
        ...
        <div className="grid grid-cols-1 gap-8 sm:grid-cols-2 lg:grid-cols-4">
          <div className="space-y-4">
            <div className="flex items-center gap-2">
              <BrandMark className="h-8 w-8 text-emerald-300" />
              <span className="text-lg font-bold text-white">
                High Society <span className="text-emerald-300">MN</span>
              </span>
            </div>
            ...
            <div className="flex gap-3">
              <a
                ...
                className="text-sm font-medium text-white/48 transition-colors hover:text-emerald-300"
            ...
            <ul className="space-y-2">
              {[
                ...
                <li key={link.href}>
                  <Link
                    href={link.href}
                    className="text-sm text-white/48 transition-colors hover:text-emerald-300"
                  >
                    {link.label}
                  </Link>
                </li>
