...
export function Header() {
  ...
  return (
    <header className="sticky top-0 z-50 border-b border-white/[.08] bg-[#050505]/72 shadow-[0_14px_50px_rgba(0,0,0,.3)] backdrop-blur-[32px]">
      <div className="pointer-events-none absolute left-[8%] top-[-3rem] h-20 w-72 rounded-full bg-amber-400/14 blur-[55px]" />
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="flex h-[4.75rem] items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <Link
              href="/cart"
              className="relative p-2 text-white/65 transition-colors hover:text-orange-300"
              aria-label="Cart"
            >
          ...
          <div className="px-4 py-4 space-y-1">
            {navLinks.map((link) => (
              <Link
                key={link.href}
                href={link.href}
                className="block py-2 text-sm text-white/65 hover:text-orange-300"
