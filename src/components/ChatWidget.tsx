            ...
            <div className="flex min-h-0 flex-1 flex-col">
              <div className="border-b border-slate-200 bg-slate-50 p-4">
                <form onSubmit={searchArea} className="flex gap-2">
                  <div className="relative flex-1">
                    ...
                    <input value={areaQuery} onChange={(event) => setAreaQuery(event.target.value)} placeholder="City, neighborhood, or ZIP code"
                      className="h-11 w-full rounded-xl border border-slate-300 bg-white pl-10 pr-4 text-sm outline-none focus:border-green-500" />
                  </div>
                  <button className="rounded-xl bg-green-700 px-4 text-sm font-semibold text-white">Search</button>
                ...
                <div className="mt-3 flex flex-wrap items-center gap-2">
                  ...
                  {[5, 10, 25, 50].map((distance) => <button key={distance} onClick={() => setMaxDistance(distance)} className={`rounded-full px-3 py-1 text-xs font-semibold ${maxDistance === distance ? "bg-green-700 text-white" : "border border-slate-200 bg-white text-slate-600"}`}>{distance} mi</button>)}
                  <button onClick={() => setWebsiteOnly((current) => !current)} className={`rounded-full px-3 py-1 text-xs font-semibold ${websiteOnly ? "bg-green-700 text-white" : "border border-slate-200 bg-white text-slate-600"}`}>Online menu</button>
                </div>
              </div>

              {locationState === "idle" && <div className="m-auto px-6 py-12 text-center"><Map className="mx-auto h-14 w-14 text-green-600" /><h3 className="mt-4 text-2xl font-semibold">Find your local cannabis scene.</h3><p className="mx-auto mt-2 max-w-lg text-sm leading-6 text-slate-600">Search a Minnesota city or ZIP code, or use your current location to discover mapped dispensaries ordered by distance.</p><button onClick={findNearby} className="mt-5 inline-flex items-center gap-2 rounded-xl bg-green-700 px-5 py-3 font-semibold text-white"><Navigation className="h-4 w-4" />Explore near me</button></div>}
              ...
              {locationState === "error" && <div className="m-auto py-10 text-center"><p className="text-sm text-red-600">{locationError}</p><button onClick={findNearby} className="mt-4 rounded-lg border border-green-200 px-4 py-2 text-sm font-medium text-green-700">Try current location</button></div>}
              {locationState === "done" && <div className="grid min-h-0 flex-1 lg:grid-cols-[1.05fr_0.95fr]">
                <div className="order-2 overflow-y-auto p-4 lg:order-1">
                  <div className="mb-3 flex items-end justify-between"><div><p className="text-xs font-semibold uppercase tracking-[0.18em] text-green-600">Near {locationLabel}</p><p className="mt-1 text-sm text-slate-500">{visiblePlaces.length} mapped result{visiblePlaces.length === 1 ? "" : "s"}</p></div></div>
                  <div className="space-y-3">
                    ...
                    {visiblePlaces.map((place, index) => <article key={place.id} className="rounded-2xl border border-slate-200 bg-white p-4 transition hover:border-green-300 hover:shadow-sm">
                      <div className="flex items-start gap-3"><span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-green-50 text-xs font-bold text-green-700">{index + 1}</span><div className="min-w-0 flex-1"><div className="flex items-start justify-between gap-3"><div><h3 className="font-semibold">{place.name}</h3><p className="mt-1 text-sm text-slate-500">{place.address}</p></div><span className="shrink-0 rounded-full bg-green-50 px-2.5 py-1 text-xs font-semibold text-green-700">{place.distanceMiles} mi</span></div>
                        {place.openingHours && <p className="mt-2 text-xs text-slate-500">{place.openingHours}</p>}
                        <div className="mt-3 flex flex-wrap gap-2"><a target="_blank" rel="noreferrer" href={`https://www.openstreetmap.org/directions?to=${place.latitude},${place.longitude}`} className="inline-flex items-center gap-1.5 rounded-lg bg-green-700 px-3 py-2 text-xs font-semibold text-white">Directions <Navigation className="h-3.5 w-3.5" /></a>{place.website && <a target="_blank" rel="noreferrer" href={place.website} className="inline-flex items-center gap-1.5 rounded-lg border border-slate-200 px-3 py-2 text-xs font-semibold">Menu / website <ExternalLink className="h-3.5 w-3.5" /></a>}</div>
                      </div></div>
                    </article>)}
                  </div>
                </div>
                <div className="order-1 min-h-56 border-b border-slate-200 bg-green-50 lg:order-2 lg:border-b-0 lg:border-l">
